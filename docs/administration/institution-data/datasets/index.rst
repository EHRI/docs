...
The first thing to do when visiting the Import Manager is to create a new dataset. A dataset is a *set
of files that come from the same place and are processed in the same way*. Although an institution could just have
...
Content-Type Override
  ...
  If you have a CSV file this can also be specified by using "text/csv" as the content type.
...
Tab 3: Ingest
  ...
  be valid EHRI-profile EAD XML which can be ingested into the portal database, or a CSV file matching the Portal's
  CSV header format.
